...
def main():
    ...
    page = st.sidebar.radio("Go to", ["Prediction", "Modeling", "Time-series modeling with SARIMA"])
...
def load_sarima_model(url):
    response = requests.get(url)
    response.raise_for_status()
    model = pickle.loads(response.content)
    return model

def time_series_modeling():
    st.title('Time-series modeling with SARIMA')

    # Load SARIMA model
    sarima_model = load_sarima_model('https://storage.googleapis.com/sarimapkl/sarima_model.pkl')

    ts = pd.read_csv('/mnt/data/ts.csv')
    ...
    # Model Evaluation
    predictions = sarima_model.get_forecast(steps=len(test_data)).predicted_mean
    mae = mean_absolute_error(test_data, predictions)
    mse = mean_squared_error(test_data, predictions)
    rmse = np.sqrt(mse)
    st.write(f'Mean Absolute Error: {mae}')
    ...
    # Forecasting
    forecast = sarima_model.get_forecast(steps=len(test_data))  # Forecast for the test period
    ...
    # Forecasting future values
    forecast_steps = 36  # Forecasting 3 years ahead (for monthly data)
    forecast_values = sarima_model.get_forecast(steps=forecast_steps).predicted_mean
    confidence_intervals = sarima_model.get_forecast(steps=forecast_steps).conf_int()
...
def prediction():
    ...
    # Load the DataFrame
    data = pd.read_csv("/mnt/data/datas_preprocessed.csv")
    df3 = data.copy()
...
def modeling():
    ...
    # Load data
    df3 = pd.read_csv("/mnt/data/datas_preprocessed.csv")
